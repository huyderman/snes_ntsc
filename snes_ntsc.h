...
/* Initialize and adjust parameters. Can be called multiple times on the same
   snes_ntsc_t object. */
void snes_ntsc_init( struct snes_ntsc_t*, snes_ntsc_setup_t const* setup );

/* Blit one or more scanlines of SNES 15-bit BGR pixels to 16-bit 5-6-5 RGB output. For
...
void snes_ntsc_blit( struct snes_ntsc_t const*, unsigned short const* snes_in, long in_pitch,
		int burst_phase, int out_width, int out_height, void* rgb16_out, long out_pitch );
void snes_ntsc_blit_hires( struct snes_ntsc_t const*, unsigned short const* snes_in, long in_pitch,
		int burst_phase, int out_width, int out_height, void* rgb16_out, long out_pitch );

/* Useful values to use for output width and number of input pixels read */
enum { snes_ntsc_min_out_width = 684 }; /* minimum width that doesn't cut off active area */
enum { snes_ntsc_min_in_width  = 256 };

enum { snes_ntsc_full_out_width = 636 }; /* room for some overscan while fitting within 640 */
enum { snes_ntsc_full_in_width  = 274 }; /* extra pixels need to be set to black */

/* private */
enum { snes_ntsc_entry_size = 144 };
enum { snes_color_count = 0x2000 };
typedef unsigned long ntsc_rgb_t;

/* Caller must allocate space for blitter data, which uses over 4500 KB of memory. */
typedef struct snes_ntsc_t
{
	ntsc_rgb_t table [snes_color_count] [snes_ntsc_entry_size];
} snes_ntsc_t;
